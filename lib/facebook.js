...
var crypto, qs, request, Mongoose, Underscore, async, Utilities, Storage, Promise, Person, graph_url;
...
exports.postAction = function (fb_id, access_token, namespace, action, actioninfo, callback) {
  request({
    url: graph_url + '/' + fb_id + '/' + namespace + ':' + action,
    qs: Underscore.extend({ access_token: access_token }, actioninfo),
    method: 'POST'
...
exports.requestDialog = function(fb_app_id) {
  return "https://www.facebook.com/dialog/apprequests" +
    "?app_id=" + fb_app_id +
    "&display=" + "popup" +
    "&message=" + encodeURIComponent("Invite some chumps!") +
