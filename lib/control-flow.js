...
function respond_with_guess (req, res) {
  ...
  fs.readFile( 'views/guess.html', function(err, contents) {
    ...
    get_unguessed_fragment( res.metastore.person, function(err, fragment) {
      ...
      res.send( Mustache.to_html( String(contents), {
        ...
        entity_actioned: fragment.entity.type_actioned,
 ...
 * prefer fragment authored by friend
 * don't see same fragment twice
 */
function get_unguessed_fragment (person, callback) {
  Person.findById(person._id, ['fb_friends'], function(err, person) {
    if (err) { callback(err, null); return; }
    fetchFragment(person, true, function(err, fragment) {
      if (err || !fragment) {
        fetchFragment(person, false, function(err, fragment) {
          callback(err, fragment);
        });
      } else {
        callback(err, fragment);
      }
...
function fetchFragment (person, onlyfriends, callback) {
  var fragments_in_votes, query, i;
  Vote.find( { voter: person._id }, ['fragment'], function(err, votes) {
    ...
    query.ne('author', person._id);       // no fragments authored by person
    if (onlyfriends) { query.in('author', person.fb_friends); } // friends
    query.nin('_id', fragments_in_votes); // no fragments previously voted on
    query.run(function(err, count) {
      if (count === 0) { callback(null, null); return; }
      i = Utilities.randInt(count);
      query = Fragment.findOne();
      query.ne('author', person._id);
      if (onlyfriends) { query.in('author', person.fb_friends); } // friends
      query.nin('_id', fragments_in_votes);
      query.skip(i);
      query.populate('entity');
      query.run(function(err, fragment) {
        if (err) { callback(err, null); return; }
