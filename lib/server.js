...
var fs, path, express, Mongoose, invocation, Store, Points, log_dir;
...
server.configure(function() {
  ...
  server.use(require('./control-auth').enforcer({
    // pages without auth protection
    whitelist: [ '^/([?#].*)?$', '^/login', '^/logout', '^/scripts', '^/styles', '^/images' ]
  }));
  server.use(Points.fetch_points_middleware);
