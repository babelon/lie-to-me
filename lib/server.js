...
server.get('/', function(req, res) {
  fs.readFile( 'views/index.html', function(err, contents) {
    if (err) { res.json( { msg: 'fs error', err: err }, 500); }
      ...
      if (req.session.oauth_access_token && req.session.oauth_access_token) {
        // logged in
...
server.get('/login', function(req, res) {
  var now = new Date();
  if (req.session['oauth_access_token'] && req.session['oauth_token_expires'] >= now) {
    res.redirect('/');
      ...
      request.get( 'https://graph.facebook.com/oauth/access_token?' +
        ...
        function(err, response, body) {
          ...
          if (err || response_body['error']) { res.json( { msg: 'fb login error', err: err || response_body['error'] }, 500); }
            ...
            req.session['oauth_access_token'] = response_body['access_token'];
            var secs_to_expire = Number(response_body['expires']);
            var expires = ( Date.now() / 1000 + secs_to_expire ) * 1000;
            req.session['oauth_token_expires'] = new Date( expires );
                    ...
                    if (err) { res.json( { msg: 'db retrieval error', err:err }, 500 ); }
                      ...
                      if (!person) {
                        ...
                        var person = new Person({
                          ...
                          oauth_token_expires: req.session['oauth_token_expires']
                        });
                      } else {
                        // Log access_token for existing user
                        person['oauth_provider'] = 'facebook';
                        person['oauth_access_token'] = req.session['oauth_access_token'];
                        person['oauth_token_expires'] = req.session['oauth_token_expires'];
                      }
