      ...
      fs.readFile( 'views/compose.html', function(err, contents) {
        ...
        Entity.findById( req.query['entity_id'], function(err, entity) {
          ...
          if ( req.query['experience'].match( /true/i ) ) {
            ...
            stars_snippet = render_stars(statement_truth, 1);
          } else {
            var rating;
            statement_truth = false;
            // uses uniform probability for now
            rating = Utilities.pickOne([1,2,3,4,5]);
            statement_requested = Mustache.to_html( Sitevars[ 'star_' + String(rating) + '_statement_request' ],
              { entity: entity.html_snippet, actioned: Sitevars.actioned_types[ entity.etype ] });
            stars_snippet = render_stars(statement_truth, rating);
          }
...
function render_stars (authentic, rating) {
  var snippet, star, range;
