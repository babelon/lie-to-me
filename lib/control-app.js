    ...
    fs.readFile( 'views/compose.html', function(err, contents) {
      ...
      if (req.query['experience'] && req.query['entity_id']) {
        ...
        Entity.findById( req.query['entity_id'], function(err, entity) {
          var statement_requested;
          if (err) { res.json( { msg: 'db error', err: err }, 500); return; }
          if ( req.query['experience'].match( /true/i ) ) {
            statement_requested = Sitevars.true_statement_request;
          } else {
            if (Math.random() < Sitevars.prob_request_positive_statement) {
              statement_requested = Sitevars.positive_statement_request;
            } else {
              statement_requested = Sitevars.negative_statement_request;
            }
          }
          statement_requested += ' <a href="' + entity.website + '">' + entity.name + '</a>';
          res.contentType('text/html');
          res.send( Mustache.to_html( String(contents), {
            truthknown: true,
    ...
    vote.save(function(err) {
      ...
      Fragment.findById( vote.fragment, function(err, fragment) {
        ...
        if (fragment.truth) {
          ...
          replyText = vote.truth ? Sitevars.truth_guessed_truth : Sitevars.truth_guessed_lies;
        } else {
          replySuccess = vote.truth ? false : true;
          replyText = vote.truth ? Sitevars.lies_guessed_truth : Sitevars.lies_guessed_lies;
        }
        fs.readFile( 'views/guess.html', function(err, contents) {
          var fragment;
          if (err) { res.json( { msg: 'fs error', err: err }, 500); return; }
