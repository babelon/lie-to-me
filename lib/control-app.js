    ...
    if (req.query['experience'] && req.query['entity_id']) {
      fs.readFile( 'views/compose.html', function(err, contents) {
        ...
        Entity.findById( req.query['entity_id'], function(err, entity) {
          var title, statement_requested, statement_truth, stars_snippet;
          ...
          if ( req.query['experience'].match( /true/i ) ) {
            ...
            stars_snippet = render_stars(statement_truth, null);
          } else {
            ...
            if (Math.random() < Sitevars.prob_request_positive_statement) {
              ...
              stars_snippet = render_stars(statement_truth, true);
            } else {
              statement_requested = Mustache.to_html( Sitevars.negative_statement_request,
                { entity: entity.html_snippet, actioned: Sitevars.actioned_types[ entity.etype ] });
              stars_snippet = render_stars(statement_truth, false);
            }
          }
...
function respond_with_guess (reply, req, res) {
  fs.readFile( 'views/guess.html', function(err, contents) {
    ...
    get_unseen_fragments( req.session.person, function(err, fragments) {
      if (err) { res.json( { msg: 'db error', err: err }, 500); return; }
      if (!fragments.length) {
        if ( Sitevars.leaders_exhausted.indexOf(reply.leader) !== -1 ) {
...
function render_stars (authentic, positive) {
  Sitevars.star_filled_snippet;
  Sitevars.star_empty_snippet;
  var snippet, star, range, rating;
  ...
  if (authentic) {
    ...
    if (!positive) { rating = Math.ceil(Math.random() * 2); }
    else { rating = 2 + Math.ceil(Math.random() * 3); }
    stars[ rating - 1 ]['hidden'] = { rating: rating };
  }
  stars[ rating - 1 ]['checked'] = 'checked';
  snippet += Mustache.to_html( Sitevars.star_filled_snippet,
    { range: stars.slice(0, rating) });
  snippet += Mustache.to_html( Sitevars.star_empty_snippet,
