          ...
          if (err) { res.json( { msg: 'db saving error', err: err }, 500); }
            ...
            Fragment.findById( vote.fragment, function(err, fragment) {
              if (err) { res.json( { msg: 'db find error', err: err }, 500); }
                ...
                if (fragment.truth) {
                  if (vote.truth) {
                    replyText = Replies.truth_guessed_truth;
                  } else {
                    replyText = Replies.truth_guessed_lies;
                  }
                } else {
                  if (vote.truth) {
                    replyText = Replies.lies_guessed_truth;
                  } else {
                    replyText = Replies.lies_guessed_lies;
                  }
                }
                fs.readFile( 'views/votereply.html', function(err, contents) {
                  if (err) { res.json( { msg: 'fs error', err: err }, 500); }
                  else {
                    res.send( Mustache.to_html( String(contents), { reply: replyText }), 201 );
                  }
                });
              }
            });
