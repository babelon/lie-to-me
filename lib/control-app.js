...
function compose_reply (vote, authentic, callback) {
  ...
  if (!vote) {
    ...
    reply.text = authentic ? Sitevars.composed_authentic_fragment : Sitevars.composed_deceptive_fragment;
    callback(null, reply);
    return;
  }
  // come from vote flow
      ...
      Entity.findById( fragment.entity, function(err, entity) {
        ...
        if (fragment.truth) {
          ...
          if ( vote.truth ) {
            ...
            reply.text = Sitevars.truth_guessed_truth;
          } else {
            reply.leader = Utilities.pickOne( Sitevars.leaders_wrong );
            reply.text = Sitevars.truth_guessed_lies;
          }
        } else {
          reply.alert_type = Sitevars.reply_alert_type[ String(!vote.truth) ];
          if ( vote.truth ) {
            reply.leader = Utilities.pickOne( Sitevars.leaders_wrong );
            reply.text = Sitevars.lies_guessed_truth;
          } else {
            reply.leader = Utilities.pickOne( Sitevars.leaders_right );
            reply.text = Sitevars.lies_guessed_lies;
          }
        }
