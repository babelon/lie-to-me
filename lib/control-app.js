...
function compose_reply (from, what, callback) {
  ...
  if (from === 'exhausted') {
    ...
    fetch_reply(alert_type, leader, tweener, text, callback);
    return;
  ...
  .run(function(err, fragment) {
    ...
    fetch_reply(alert_type, leader, tweener, text, function(err, reply) {
      reply.text = Mustache.to_html(reply.text,
        { author: fragment.author.html_snippet, entity: fragment.entity.html_snippet, actioned: fragment.entity.type_actioned });
      callback(null, reply);
    });
  });
}

function fetch_reply (alert_type, leader, tweener, text, callback) {
  fetches = Store.multi();
...
function respond_with_guess (reply, req, res) {
  fs.readFile( 'views/guess.html', function(err, contents) {
    ...
    get_unguessed_fragment( req.session.person, function(err, fragment) {
      if (err) { res.json( { msg: 'db error', err: err }, 500); return; }
      if (!fragment) {
        if ( reply && reply.text.indexOf("Looks like you've already") ) {
