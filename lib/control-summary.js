...
exports.since_accessed = function (req, res) {
  ...
  promise = new Promise();
  Fragment.find({ author: req.session.person._id, truth: false }, [ '_id', 'entity', 'truth' ])
  .populate('entity')
...
function fetch_summaries (votes, id_fragments_map, callback) {
  var fetches, leaders_s, leaders_f, successes, failures, summaries;
  fetches = Storage.redis('multi');
  fetches.smembers('leaders_notification_success');
  fetches.smembers('leaders_notification_failure');
  ...
  fetches.exec(function(err, replies) {
    ...
    summaries = votes.map(function(v) {
      ...
      return {
        ...
        leader: Mustache.to_html( Utilities.pickOne(successful ? leaders_s : leaders_f), {
